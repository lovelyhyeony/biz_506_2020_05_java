 ...
 * 
 * static 클래스는 초기화 과정을 수행할 필요 없이 사용할 수 있다.
 * SplitPosition sPos = new SplitPosition(); (// 초기화 과정) 와 같은 코드를 사용할 필요가 없다
 * 
 * static 클래스는 프로젝트가 run(시작)되는 순간
 * 자동으로 객체를 **1번** 생성하여 method 기억영역에 보관해 둔다
 * 1번만 생성되기 때문에 메모리 공간을 1번만 차지하는 효과를 내고
 * 프로젝트의 어떠한 클래스, method에서든지 직접 접근하여 사용할 수 있다.
 */
public class SplitPosition {

	// static으로 선언된 변수는
	// 누구나 어디에서든지 접근하여 읽기, 쓰기가 가능하다.
	// 만약 WORD_ENG 변수를 어딘가에서 0이아닌 다른 값으로 변경을 해버리면
	// WORD_ENG변수를 진짜 사용해야 할 곳에서 문제가 발생할 수 있다.
	// 그래서 이러한 변수에는 final 키워드를 붙여준다.
	// final 키워드가 부착된 변수는 다른 곳에서 절대 읽기만 가능하고
	// 절대 값을 변경할 수 없게 된다
	
	// static으로 만든 변수들은 일반적으로final이랑 같이 사용함.
