...
// 클래스를 이용한 코딩
	public static void main(String[] args) {
		...
		// Score 클래스를 사용해서
		// scores 라는 인스턴스 배열을 3개 선언만하는 코드
		...
		for(int i = 0 ; i < 3 ; i++) {
			...
			System.out.printf("%s\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n",scores[i].name,scores[i].intKor,scores[i].intEng,scores[i].intMath,scores[i].intMusic,scores[i].intArt,scores[i].sum(),scores[i].avg());
