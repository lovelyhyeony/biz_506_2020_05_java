...
public class ScoreEx_03 {
	...
	public static void main(String[] args) {
		...
		// Integer형 데이터들을 포함할 리스트
 		...
 		// java 1.8 이상에서는
 		// 초기화 코드(new ArrayList)에는 Generic을 생략할 수 있다. // =앞에 같다고 생각함
 		List<ScoreVO> score18 = new ArrayList<>();
 		
 		// 개발자가 작성한 ScoreVO클래스 type의 인스턴스를 
 		// 포함할 수 있는 리스트
 		List<ScoreVO> scoreList = new ArrayList<ScoreVO>();
 		
 		// jdk에서 제공되는 Random 클래스 type의 인스턴스를
 		// 포함할 수 있는 리스트
 		List<Random> rndList = new ArrayList<Random>(); // 어떤 클래스도 사용할 수 있음 //jdk에서 불러도 되고 개발자가 만든 클래스도 됌
